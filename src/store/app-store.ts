...
interface GraphState {
  ...
  loading: boolean;
  graphRepoKey: string | null; // Format: "owner/repo/branch" - tracks which repo's graphData is cached
...
interface SavedHistoryState {
  ...
  setSavedHistoryItems: (item: { [date: string]: Array<HistoryItem> }) => void; //Takes one item to add to the list
  ...
  devtools(
    persist(
      (set) => ({
        ...
        setGraphData: (data: EcosystemGraphMap, graphRepoKey?: string) =>
        ...
        // Saved History State
        savedHistoryItems: {},
        setSavedHistoryItems: (item: { [date: string]: Array<HistoryItem> }) =>
          set((s) => ({
