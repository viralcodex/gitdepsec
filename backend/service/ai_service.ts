...
class AiService {
  ...
  constructor(model?: string, apiKey?: string) {
    ...
    this.defaultModel = model ?? process.env.DEFAULT_MODEL ?? "mistralai/devstral-2512:free";
  }

  generateVulnerabilitySummary(vulnerabilities: Vulnerability[], model?: string): Promise<string> {
  ...
  generateInlineResponse(
    ...
    console.log("Generating inline response with model:", model ?? this.defaultModel);
    return AIUtils.callAI(this.ai, systemPrompt, fullUserPrompt, inlineaiSummarySchema, {
