...
describe("AgentsServiceNew", () => {
  ...
  describe("Phase 0: Smart Preprocessing", () => {
    it("should flatten dependencies correctly", () => {
      ...
      const directDeps = flattenedData.filter((d: any) => d.dependencyLevel === "direct");
      expect(directDeps.length).toBe(3); // lodash, axios, express

      // Check transitive dependencies
      const transitiveDeps = flattenedData.filter((d: any) => d.dependencyLevel === "transitive");
  ...
  describe("Rollback Procedures", () => {
    it("should generate rollback commands", () => {
      const generateRollbackProcedures = (service as any).generateRollbackProcedures.bind(service);
  ...
  describe("Integration: Full Workflow", () => {
    it("should track progress through all phases", async () => {
      ...
      expect(progressEvents.some((e) => e.step === "synthesis_complete")).toBe(true);
      expect(progressEvents.some((e) => e.step === "enrichment_start")).toBe(true);
      expect(progressEvents.some((e) => e.step === "enrichment_complete")).toBe(true);
    });
    ...
    it("should include detailed metadata in final output", async () => {
      ...
      expect((result.metadata as any).total_packages_analyzed).toBeGreaterThan(0);
      expect(result.automated_execution).toBeDefined();
      expect((result.automated_execution as any).cli_commands).toBeDefined();
      expect((result.automated_execution as any).rollback_procedures).toBeDefined();
      expect((result.automated_execution as any).one_click_script).toBeDefined();
    });
  });
