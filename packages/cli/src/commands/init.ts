...
export async function initCommand(options: InitCommandOptions): Promise<void> {
    ...
    console.log(`${chalk.green("[ok]")} Created ${chalk.bold(".gitdepsecrc")}`);
    console.log("");
    console.log(chalk.dim("Configuration options:"));
    console.log(chalk.dim("─".repeat(40)));
    const showConfigLine = (key: string, description: string) => {
        console.log(`  ${chalk.cyan(`${key}:`.padEnd(21, " "))}${description}`);
    };
    showConfigLine("github_token", "GitHub PAT for private repos");
    showConfigLine("include_transitive", "Scan transitive deps");
    showConfigLine("output_format", "table | json | markdown");
    console.log("");
